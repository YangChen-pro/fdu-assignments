...
for model in "${models[@]}"; do
  for mode in "${modes[@]}"; do
    ...
    metrics="results/task_a/vllm/metrics/${name}_${mode}_test_metrics.csv"
    raw="results/task_a/vllm/raw_outputs/${name}_${mode}_test_raw_outputs.jsonl"
    batch_size=128
    if [[ "${mode}" == "thinking" ]]; then
      batch_size=32
    fi
    if [[ -f "${metrics}" ]] && [[ -f "${raw}" ]] && [[ "$(wc -l < "${raw}")" -eq 3000 ]]; then
      echo "===== $(date '+%F %T') SKIP model=${model} mode=${mode} existing=${metrics} ====="
      continue
    fi
    echo "===== $(date '+%F %T') START model=${model} mode=${mode} batch_size=${batch_size} ====="
    ...
    status=$?
    if [[ "${status}" -ne 0 ]]; then
      echo "===== $(date '+%F %T') FAIL model=${model} mode=${mode} status=${status} ====="
      exit "${status}"
    fi
    echo "===== $(date '+%F %T') DONE model=${model} mode=${mode} ====="
  done
done
