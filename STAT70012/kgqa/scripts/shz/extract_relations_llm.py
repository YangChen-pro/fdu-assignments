...
# Config (edit these)
# =========================
BOOK_PATH = Path("kgqa/data/input/shz.txt")

OUTPUT_DIR = Path("kgqa/data/output")
OUT_RELATIONS_JSONL = OUTPUT_DIR / "shz_new_relations.jsonl"
EXTRACT_LOG_JSONL = OUTPUT_DIR / "shz_extract_log.jsonl"
NORMALIZE_LOG_JSONL = OUTPUT_DIR / "shz_normalize_log.jsonl"
VERIFY_LOG_JSONL = OUTPUT_DIR / "shz_verify_log.jsonl"

# Final export: 7 columns like relation_gemini.txt
EXPORT_TXT_PATH = Path("kgqa/raw_data/shz_relation_llm.txt")
...
RUN_SIGNATURE = f"shz_v1|limit={CHUNK_TOKEN_LIMIT_EST}|overlap={CHUNK_OVERLAP_CHARS}|multipass=1"
...
EXTRACT_PASSES: list[dict[str, Any]] = [
    {
        "name": "pp_social",
        "max_relations": 15,
        "focus": "优先抽取 人物-人物 的社会关系与互动：结拜/兄弟/好汉/头领/师徒/敌对/追杀/擒拿/斩杀/救助/招安/入伙/投靠/背叛等。"
    },
    {
        "name": "title_office",
        "max_relations": 15,
        "focus": "优先抽取 封号/官职/职位/任命/受封/册封/排座次/坐第几把交椅/担任/自称等制度性关系。"
    },
    {
        "name": "battle_event",
        "max_relations": 15,
        "focus": "优先抽取 战斗/事件 相关：劫狱/打虎/醉打/大闹/火烧/攻打/防守/夺取/占据/剿匪/征讨等。"
    },
    {
        "name": "item_place",
        "max_relations": 12,
        "focus": "优先抽取 人物-兵器/武器 以及 人物-地点 的关系：使用兵器/驻守/前往/镇守/落草/上梁山/下山/出生于/死于等。"
    },
]
...
def _infer_cate(*, name_norm: str, name_raw: str, ent_type: str) -> str:
    """
    Rule-first category inference for 水浒传.
    ...
    # 梁山好汉
    if any(k in n for k in ["梁山", "水泊", "聚义厅", "忠义堂"]):
        return "梁山好汉"

    # 朝廷官军
    if any(k in n for k in ["高俅", "童贯", "蔡京", "太尉", "太师", "枢密使"]):
        return "朝廷官军"

    # 地方官府
    if any(k in n for k in ["知府", "知州", "知县", "县令", "太守", "提辖", "都监", "教头"]):
        if ent_type == "Person":
            return "地方官府"

    # 地点相关
    if ent_type == "Place":
        if any(k in n for k in ["东京", "汴梁", "大名府", "沧州", "登州", "府", "州", "县"]):
            return "地方官府"
        if any(k in n for k in ["梁山泊", "水泊"]):
            return "梁山好汉"

    # 方腊势力
    if any(k in n for k in ["方腊", "王庆", "田虎", "石宝"]):
        return "方腊势力"

    # 辽国势力
    if any(k in n for k in ["辽", "辽国", "契丹"]):
        return "辽国势力"
...
def _build_extract_messages(
    *,
    ...
    system = (
        "你是信息抽取助手。你的任务是从给定《水浒传》文本中抽取关系三元组。"
    ...
    user = (
        ...
        "\n"
        "字段要求：\n"
        "- head 与 tail 必须是具体专名（人名/绰号/地名/州府/兵器等），不要用代词或泛称（他/她/哥哥/兄弟/头领/好汉等）。不确定就不要输出。\n"
        "- 重要：head 与 tail 必须是 evidence 中**原样出现**的连续子串（完全一致），不要把'及时雨'改写成'宋江'，不要补全未出现在 evidence 的字。\n"
        "- relation 为'谓词短语'，开放集合：\n"
        "  * 必须简短（2~8 个中文为佳，最长不超过 12 个字）\n"
        "  * 不要包含逗号/换行/冒号/引号\n"
        "  * 不要输出整句或'相关/有关/提到/出现/发生'等泛词\n"
        "  * relation 里不要包含 head/tail 的名字或别名（例如不要输出'打死李逵'，应输出'打死'）\n"
        "- head_type/tail_type 必须选择枚举值。\n"
        "- head_cate/tail_cate 必须选择枚举值；尽量按《水浒传》体系归类（梁山好汉/朝廷官军/地方官府等）。\n"
...
def _build_normalize_messages(*, items: list[dict[str, Any]]) -> list[dict[str, str]]:
    ...
    user = (
        ...
        "\n"
        "类目参考（尽量按此纠偏）：\n"
        "- 宋江/卢俊义/林冲/武松/鲁智深/吴用/公孙胜/关胜/李逵/花荣/柴进/李应/朱仝/雷横/燕青/石秀：梁山好汉\n"
        "- 高俅/童贯/蔡京/杨戬/呼延灼/张叔夜：朝廷官军\n"
        "- 知府/知州/县令/太守/提辖：地方官府\n"
        "- 方腊/石宝/王庆/田虎：方腊势力、其他势力\n"
        "- 梁山泊/水泊梁山/聚义厅/忠义堂：梁山好汉\n"
        "- 东京/汴梁/大名府/沧州/登州：地方官府\n"
        "\n"
        "规范化要求（不满足就输出 null）：\n"
