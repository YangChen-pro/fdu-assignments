...
# Config (edit these)
# =========================
BOOK_PATH = Path("kgqa/data/input/xyj.txt")

OUTPUT_DIR = Path("kgqa/data/output")
OUT_RELATIONS_JSONL = OUTPUT_DIR / "xyj_new_relations.jsonl"
EXTRACT_LOG_JSONL = OUTPUT_DIR / "xyj_extract_log.jsonl"
NORMALIZE_LOG_JSONL = OUTPUT_DIR / "xyj_normalize_log.jsonl"
VERIFY_LOG_JSONL = OUTPUT_DIR / "xyj_verify_log.jsonl"

# Final export: 7 columns like relation_gemini.txt
EXPORT_TXT_PATH = Path("kgqa/raw_data/xyj_relation_llm.txt")
...
CHUNK_OVERLAP_CHARS = 350  # small overlap to reduce cross-chunk relation loss (set 0 to disable)

# Change this when chunking / parsing / prompts change, to avoid skipping old done chunks.
RUN_SIGNATURE = f"xyj_v3|limit={CHUNK_TOKEN_LIMIT_EST}|overlap={CHUNK_OVERLAP_CHARS}|multipass=1"
...
EXTRACT_PASSES: list[dict[str, Any]] = [
    {
        ...
        "max_relations": 12,
        "focus": (
            "优先抽取 人物-人物 的社会关系与互动：师徒/拜师/结义/盟友/敌对/追杀/擒拿/救助/告状/奏请/奉命捉拿/战败/挑战等。"
        ),
    },
    {
        "name": "title_office",
        "max_relations": 12,
        "focus": (
            "优先抽取 封号/官职/任命/受封/册封/加封/封为/任命为/奏请封为/传旨 等制度性关系。"
        ),
    },
    {
        "name": "item_skill",
        "max_relations": 12,
        "focus": (
            "优先抽取 人物-物品/法宝/兵器/丹药/书册 以及 人物-技能/法术 的关系：索要/获得/赠与/献上/偷取/使用/施展/变作/化为等。"
        ),
    },
    {
        "name": "place_org_event",
        "max_relations": 12,
        "focus": (
            "优先抽取 地点/组织/事件 相关：位于/居住于/前往/到达/进入/驻扎/设宴/赴会/围困(发生地)等；人物参与事件可用 参与/赴会/设宴/带领前往。"
        ),
    },
]
...
_CATE_FIXED_BY_ENTITY: dict[str, str] = {
    ...
    "赤脚大仙": "天庭（天宫/星宿/天兵天将）",
    # 道门
    "太上老君": "道门仙家（三清/老君/洞天福地）",
    "须菩提祖师": "道门仙家（三清/老君/洞天福地）",
    "菩提祖师": "道门仙家（三清/老君/洞天福地）",
    # 地府
    "秦广王": "地府冥界（十殿/鬼差）",
    # 龙宫
    "东海龙王敖广": "龙宫水族（四海/水府）",
...
def _infer_cate(*, name_norm: str, name_raw: str, ent_type: str) -> str:
    """
    Rule-first category inference for 西游记.
    ...
    if any(k in n for k in ["菩萨", "佛祖", "如来", "罗汉", "金刚", "比丘", "灵山"]):
        return "佛门（灵山/观音体系）"
    if any(k in n for k in ["天庭", "天宫", "凌霄", "瑶池", "星君", "天王", "天兵", "天将", "仙女", "玉皇"]):
        return "天庭（天宫/星宿/天兵天将）"
    if any(k in n for k in ["老君", "真人", "仙翁", "道君", "祖师", "洞", "观", "宫"]):
        # '洞/观/宫' can be ambiguous for Place; keep conservative for non-Person.
        if ent_type == "Person" and any(k in n for k in ["老君", "真人", "仙翁", "道君", "祖师"]):
            return "道门仙家（三清/老君/洞天福地）"
    if any(k in n for k in ["阎王", "阎罗", "冥", "地府", "鬼", "判官", "生死簿"]):
        return "地府冥界（十殿/鬼差）"
    if any(k in n for k in ["龙宫", "龙王", "龙子", "龙女", "水府", "水族"]):
        return "龙宫水族（四海/水府）"
    if any(k in n for k in ["土地", "山神", "城隍", "河神"]):
        return "地方神祇（土地/山神/城隍/河神）"

    if any(k in n for k in ["妖", "怪", "精", "魔", "魔王"]):
        return "妖魔势力（洞府妖王）"
    if n.endswith("大王") and len(n) >= 3:
        return "妖魔势力（洞府妖王）"

    if any(k in n for k in ["唐王", "大唐", "长安", "御弟"]):
        return "人间大唐（朝廷）"
    if any(k in n for k in ["国王", "太子", "公主", "郡王", "王妃", "将军", "丞相"]):
        return "人间诸国（西域各国/地方政权）"
    # place-like suffixes
    if ent_type == "Place":
        if n.endswith("国") or n.endswith("洲") or "部洲" in n:
            return "人间诸国（西域各国/地方政权）"
        if any(k in n for k in ["瑶池", "蟠桃园", "蟠桃会", "凌霄", "天宫", "天庭", "兜率宫", "老君宫", "齐天大圣府"]):
            return "天庭（天宫/星宿/天兵天将）"
        if any(k in n for k in ["龙宫", "东海", "西海", "南海", "北海"]):
            return "龙宫水族（四海/水府）"
        if any(k in n for k in ["幽冥", "地府"]):
            return "地府冥界（十殿/鬼差）"
        if n in {"花果山", "水帘洞"}:
            return "妖魔势力（洞府妖王）"
...
def _build_extract_messages(
    *,
    ...
    system = (
        "你是信息抽取助手。你的任务是从给定《西游记》文本中抽取关系三元组。"
    ...
    user = (
        ...
        "\n"
        "字段要求：\n"
        "- head 与 tail 必须是具体专名（人名/地名/势力名/法宝名等），不要用代词或泛称（他/她/众人/师父/国王/大王/和尚/妖怪等）。不确定就不要输出。\n"
        "- 重要：head 与 tail 必须是 evidence 中**原样出现**的连续子串（完全一致），不要把“悟空”改写成“孙悟空”，不要补全未出现在 evidence 的字。\n"
        "- relation 为“谓词短语”，开放集合：\n"
        "  * 必须简短（2~8 个中文为佳，最长不超过 12 个字）\n"
        "  * 不要包含逗号/换行/冒号/引号\n"
        "  * 不要输出整句或‘相关/有关/提到/出现/发生’等泛词\n"
        "  * relation 里不要包含 head/tail 的名字或别名（例如不要输出“挑战齐天大圣”，应输出“挑战”）\n"
        "- head_type/tail_type 必须选择枚举值。\n"
        "- head_cate/tail_cate 必须选择枚举值；尽量按《西游记》体系归类（取经团队/天庭/佛门/妖魔势力等）。\n"
...
def _build_normalize_messages(*, items: list[dict[str, Any]]) -> list[dict[str, str]]:
    ...
    user = (
        ...
        "\n"
        "类目参考（尽量按此纠偏）：\n"
        "- 孙悟空/唐三藏/猪八戒/沙悟净/小白龙：取经团队\n"
        "- 玉皇大帝/太白金星/托塔李天王/哪吒/天兵天将/仙女/星君：天庭（天宫/星宿/天兵天将）\n"
        "- 太上老君/菩提祖师/须菩提祖师：道门仙家（三清/老君/洞天福地）\n"
        "- 四海龙王/龙宫/水府：龙宫水族（四海/水府）\n"
        "- 阎王/地府/判官/生死簿：地府冥界（十殿/鬼差）\n"
        "- 观音/如来/菩萨/罗汉/金刚：佛门（灵山/观音体系）\n"
        "- …大王/…魔王/妖/怪/精/魔：妖魔势力（洞府妖王）\n"
        "\n"
        "规范化要求（不满足就输出 null）：\n"
        ...
        "- relation 中不得包含 head/tail 的名字或别名；例如不要输出“挑战齐天大圣”，应输出“挑战”。\n"
