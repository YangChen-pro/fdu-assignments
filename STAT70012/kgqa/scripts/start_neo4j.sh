...
AUTH_MODE="${AUTH_MODE:-none}"          # 可改：basic 或 none（none = 不要密码）
NEO4J_USER="${NEO4J_USER:-neo4j}"      # basic 时使用
NEO4J_PASSWORD="${NEO4J_PASSWORD:-}"    # basic 时必须由环境变量传入
...
if [[ "${AUTH_MODE}" == "none" ]]; then
  ...
  : "${NEO4J_PASSWORD:?启用 basic 认证时必须设置 NEO4J_PASSWORD}"
