...
def retrieve_memories(
    ...
    for memory in memories:
        memory_vector = np.asarray(memory["embedding"], dtype=float)
        memory_norm = np.linalg.norm(memory_vector)

        if query_norm == 0.0 or memory_norm == 0.0:
            semantic_score = 0.0
        else:
            semantic_score = float(np.dot(query_vector, memory_vector) / (query_norm * memory_norm))

        lexical_score = lexical_overlap(query, memory["text"])
        importance_score = float(memory["importance"])
        score = alpha * semantic_score + beta * lexical_score + gamma * importance_score

        scored_memory = dict(memory)
        scored_memory.update(
            score=score,
