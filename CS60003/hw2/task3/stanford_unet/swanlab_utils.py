"""Optional SwanLab logging for HW2 Task3."""
...
class SwanLabLogger:
    """Small wrapper that keeps SwanLab optional in code paths."""
    ...
    def enabled(self) -> bool:
        """Return whether a SwanLab run is active."""
        return self._module is not None

    def log(self, metrics: dict[str, Any], step: int | None = None) -> None:
        """Log scalar metrics if SwanLab is active."""
        if self.enabled:
            self._module.log(_scalar_metrics(metrics), step=step)

    def log_image(self, key: str, image_path: Path, caption: str) -> None:
        """Log one image artifact if SwanLab is active."""
        if self.enabled and image_path.is_file():
...
def create_swanlab_logger(config: dict[str, Any], run_dir: Path) -> SwanLabLogger:
    ...
    if mode == "cloud" and not api_key:
        raise RuntimeError("SwanLab cloud logging requires SWANLAB_API_KEY or the project key in .helloagents.")
    if api_key:
        swanlab.login(api_key=api_key)

    swanlab.init(
        project=settings.get("project") or "cs60003-hw2-task3",
